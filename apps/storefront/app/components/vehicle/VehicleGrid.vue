...
<template>
  <div v-if="loading" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fade-in">
    <div v-for="i in 6" :key="i" class="bg-slate-900/40 border border-slate-800 rounded-xl overflow-hidden animate-pulse h-[380px]">
  ...
  <div v-else-if="vehicles.length === 0" class="flex flex-col items-center justify-center text-center py-16 px-4 bg-slate-900/20 rounded-xl border border-slate-800/40 animate-fade-in">
    <div class="w-16 h-16 bg-slate-800/50 rounded-full flex items-center justify-center text-slate-400 mb-4 border border-slate-700/50">
      <iconify-icon icon="tabler:car-off" class="text-3xl block"></iconify-icon>
    </div>
    <h3 class="text-lg font-bold text-slate-200 mb-1">{{ t('empty.title') }}</h3>
