...
<template>
    <form @submit.prevent="submitProposal" class="space-y-4 text-left">
        <div
            v-if="success"
            class="bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 p-6 rounded-xl text-center text-sm animate-pulse"
        >
            <iconify-icon icon="tabler:circle-check" class="text-3xl mb-2 inline-block text-emerald-400"></iconify-icon>
            <p class="font-semibold text-base mb-1">{{ t('detail.form.registered') }}</p>
            <p class="text-slate-400 text-xs">{{ t('detail.success') }}</p>
        </div>

        <div v-else class="space-y-4">
            <div>
                <label for="lead-name" class="block text-xs font-normal text-slate-400 mb-1.5">
                    {{ t('detail.form.name') }}
                </label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label for="lead-email" class="block text-xs font-normal text-slate-400 mb-1.5">
                        {{ t('detail.form.email') }}
                    </label>
                    <UiInput id="lead-email" type="email" v-model="email" placeholder="joao@example.com" required />
                </div>
                <div>
                    <label for="lead-phone" class="block text-xs font-normal text-slate-400 mb-1.5">
                        {{ t('detail.form.phone') }}
                    </label>
            ...
            <div>
                <label for="lead-message" class="block text-xs font-normal text-slate-400 mb-1.5">
                    {{ t('detail.form.message') }}
                </label>
                <UiInput
                    id="lead-message"
                    type="textarea"
