        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            >
                <iconify-icon
                    ...
                    class="text-base group-hover:-translate-x-0.5 transition-transform block"
                ></iconify-icon>
                {{ t('button.back') }}
            </button>
            ...
            <div v-if="vehicle" class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                ...
                <div class="lg:col-span-2 space-y-6 text-left">
                    <!-- Main Image -->
                    <div
                        class="relative rounded-2xl overflow-hidden border border-slate-900 bg-slate-900 aspect-video shadow-2xl"
                    >
                        <img
                            :src="vehicle.image"
                    ...
                    <UiCard class="p-6 md:p-8 border-slate-800 bg-slate-900/40">
                        <div class="flex items-center gap-2 mb-4">
                            ...
                            <span class="text-xs font-semibold text-slate-450">{{ vehicle.year }}</span>
                        </div>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8 border-t border-b border-slate-850 py-6">
                            <div>
                                <span class="text-[10px] text-slate-500 block uppercase font-normal">
                                    {{ t('specs.mileage.label') }}
                                </span>
                            ...
                            <div>
                                ...
                                <span class="text-sm font-semibold text-slate-200 mt-1 block">{{ vehicle.fuel }}</span>
                            </div>
                            <div>
                                <span class="text-[10px] text-slate-500 block uppercase font-normal">
                                    {{ t('specs.color') }}
                        ...
                        <!-- Features details -->
                        <h4 class="font-semibold text-slate-350 text-sm mb-4">{{ t('specs.highlights') }}</h4>
                        <div class="flex flex-wrap gap-2">
                            <UiBadge variant="neutral">{{ t('specs.direction') }}</UiBadge>
                            <UiBadge variant="neutral">{{ t('specs.air') }}</UiBadge>
                            <UiBadge variant="neutral">{{ t('specs.windows') }}</UiBadge>
                            <UiBadge variant="neutral">{{ t('specs.multimedia') }}</UiBadge>
                            <UiBadge variant="neutral">{{ t('specs.abs') }}</UiBadge>
                            <UiBadge variant="neutral">{{ t('specs.airbags') }}</UiBadge>
                        </div>
                    </UiCard>
                </div>
                ...
                <div class="lg:col-span-1 space-y-6 text-left">
                    ...
                    <UiCard class="p-6 md:p-8 border-slate-800 bg-slate-900/60 shadow-lg">
                        ...
                        <div class="mt-6 pt-6 border-t border-slate-850">
                            <h4 class="font-semibold text-slate-200 text-sm mb-1">{{ t('detail.title') }}</h4>
                            <p class="text-xs text-slate-550 mb-6">{{ t('detail.proposal.subtitle') }}</p>
            ...
            <div v-else class="text-center py-20">
                <iconify-icon
                    ...
                    class="text-5xl text-slate-600 mb-4 block mx-auto animate-bounce"
                ></iconify-icon>
