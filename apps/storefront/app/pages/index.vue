...
const { currentStore, searchQuery, filteredVehicles, selectedVehicle, openVehicleDetails, closeDetails, formatPrice } =
    useTenant();
    ...
    <div>
        ...
        <section class="py-12 bg-slate-950 border-t border-slate-900/40">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <!-- Vehicle Grid -->
                <VehicleGrid :vehicles="filteredVehicles" @select-vehicle="openVehicleDetails" />
            </div>
        </section>
        ...
        <div
            ...
            class="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-fade-in"
        >
            <div
                class="bg-slate-900 border border-slate-800 rounded-2xl max-w-3xl w-full overflow-hidden shadow-2xl relative text-left"
            >
                <!-- Close Button -->
                <button
                    @click="closeDetails"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2">
                    ...
                    <div class="relative bg-slate-950 min-h-[260px] md:min-h-full">
                        <img
                            ...
                            @error="handleImageError($event, `${selectedVehicle.brand} ${selectedVehicle.model}`)"
                            class="w-full h-full object-cover min-h-[260px] md:h-full"
                        />
                        <div
                            class="absolute inset-0 bg-gradient-to-t from-slate-950/90 via-slate-950/30 to-transparent flex flex-col justify-end p-6"
                        >
                            <span class="text-xs font-normal text-slate-450 uppercase tracking-widest">
                                {{ selectedVehicle.brand }}
                            </span>
                            <h4 class="text-2xl font-black text-slate-100 mt-1 leading-tight">
                            ...
                            <div class="mt-6 border-t border-slate-850 pt-4">
                                <span class="text-xs text-slate-500 block">{{ t('detail.price_label') }}</span>
                                <span class="text-2xl font-extrabold store-text-color">
                    ...
                    <div class="p-6 md:p-8 bg-slate-900">
                        <div class="mb-6">
                            <h5 class="text-lg font-semibold text-slate-200">{{ t('detail.title') }}</h5>
                            <p class="text-xs text-slate-550 mt-1">{{ t('detail.form.subtitle') }}</p>
                        </div>
