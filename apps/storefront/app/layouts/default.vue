...
<template>
  <div :style="storeStyles" class="min-h-screen bg-slate-950 text-slate-100 font-['Outfit'] antialiased flex flex-col justify-between selection:bg-amber-500/30">
    <!-- Main Shell -->
      ...
      <header class="sticky top-0 z-40 bg-slate-950/80 backdrop-blur-md border-b border-slate-900/80">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          ...
          <NuxtLink to="/" class="flex items-center gap-3 group">
            <span class="p-2.5 rounded-xl bg-gradient-to-br from-slate-900 to-slate-800 border border-slate-800 text-2xl transition duration-300">
            ...
            <div class="text-left">
              ...
              <span class="text-[10px] text-slate-500 uppercase tracking-widest font-semibold mt-1 block">
          ...
          <nav class="hidden md:flex items-center gap-8 text-sm font-semibold text-slate-400">
            <NuxtLink to="/" class="hover:text-slate-100 transition duration-205" active-class="store-active-nav">
              {{ t('nav.home') }}
            </NuxtLink>
            <NuxtLink to="/about" class="hover:text-slate-100 transition duration-205" active-class="store-active-nav">
              {{ t('nav.about') }}
            </NuxtLink>
            <NuxtLink to="/contact" class="hover:text-slate-100 transition duration-205" active-class="store-active-nav">
              {{ t('nav.contact') }}
            </NuxtLink>
          </nav>
          ...
          <div class="flex items-center gap-3">
            <a
              ...
              target="_blank"
              class="inline-flex items-center px-4 py-2.5 rounded-lg text-xs font-bold bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-300 hover:text-white transition duration-200 cursor-pointer"
            >
              <iconify-icon icon="tabler:brand-whatsapp" class="text-base mr-2 text-emerald-400 block"></iconify-icon>
    ...
    <footer class="bg-slate-950 border-t border-slate-900/60 py-8">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center md:flex md:justify-between md:items-center text-slate-500 text-xs gap-4">
        <div class="flex flex-col md:items-start items-center gap-2 mb-4 md:mb-0">
          <p class="font-bold text-slate-350">Concessionária Integrada AutoHub</p>
    ...
    <div class="fixed bottom-6 right-6 z-50">
      <div class="bg-slate-900/90 backdrop-blur-md border border-slate-800 rounded-2xl shadow-xl shadow-slate-950/80 p-4 max-w-[280px] text-left">
        <div class="flex items-center gap-2 mb-2 pb-2 border-b border-slate-800">
          ...
          <span class="text-xs font-bold text-slate-200">Simulador de Loja</span>
        </div>
        <p class="text-[10px] text-slate-400 mb-3 leading-relaxed">
          Selecione a concessionária simulando o host de requisição:
        </p>
        <div class="space-y-1.5">
          <button
            ...
            :class="[
              'w-full flex items-center justify-between px-2.5 py-1.5 rounded-lg text-xs font-bold transition duration-200 cursor-pointer',
