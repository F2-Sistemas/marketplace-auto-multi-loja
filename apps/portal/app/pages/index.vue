...
<template>
  <div class="space-y-16 pb-16">
    ...
    <section class="relative bg-neutral-900 overflow-hidden py-24 md:py-32 text-center text-white">
      ...
      <div class="absolute inset-0 bg-gradient-to-tr from-brand-900/80 via-neutral-950/90 to-brand-800/40 z-0"></div>
      <div class="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-white/5 via-transparent to-transparent opacity-60"></div>
      
      <div class="container max-w-7xl mx-auto px-4 md:px-6 relative z-10 space-y-8">
        <!-- Floating brand badge -->
        <div class="inline-flex items-center gap-1.5 px-3 py-1 bg-white/10 backdrop-blur-xs rounded-full border border-white/10 text-xs font-bold uppercase tracking-wider text-brand-300">
        ...
        <div class="space-y-4 max-w-3xl mx-auto">
          ...
          <p class="text-base md:text-lg text-neutral-300 font-semibold max-w-xl mx-auto leading-relaxed">
            {{ t('hero.subtitle') }}
          </p>
        </div>
    ...
    <section class="container max-w-7xl mx-auto px-4 md:px-6">
      <div class="p-6 bg-white border border-neutral-150 rounded-card shadow-sm flex flex-wrap items-center justify-center gap-4">
        <span class="text-xs font-bold text-neutral-400 uppercase tracking-wider mr-2">Busque por marcas</span>
        <NuxtLink
          v-for="brand in ['Chevrolet', 'Fiat', 'Ford', 'Honda', 'Hyundai', 'Jeep', 'Toyota', 'Volkswagen']"
          :key="brand"
          :to="`/veiculos?brand=${brand}`"
          class="px-4 py-2 border border-neutral-200 hover:border-brand-500 rounded-button bg-white text-xs font-extrabold text-neutral-600 hover:text-brand-700 transition cursor-pointer select-none"
        >
          {{ brand }}
        </NuxtLink>
      </div>
    ...
    <section class="container max-w-7xl mx-auto px-4 md:px-6 space-y-6">
      <div class="flex items-end justify-between">
        <div class="space-y-1">
          <span class="text-xs font-bold uppercase tracking-wider text-brand-600">Oportunidades</span>
    ...
    <section class="container max-w-7xl mx-auto px-4 md:px-6 space-y-6">
      <div class="space-y-1">
        <span class="text-xs font-bold uppercase tracking-wider text-brand-600">Parceiros</span>
        <h2 class="text-2xl md:text-3xl font-black text-neutral-800 tracking-tight">
