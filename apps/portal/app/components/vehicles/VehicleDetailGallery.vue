...
<template>
    <div class="space-y-4">
        <!-- Big Main Stage -->
        <div
            class="relative aspect-video w-full rounded-card overflow-hidden bg-neutral-100 border border-neutral-200 group/gallery shadow-card"
        >
            <Transition name="fade-gallery" mode="out-in">
                <img
                    ...
                    :src="activeImage"
                    :alt="title"
                    @error="handleImageError($event, title)"
            ...
            <div
                ...
                class="absolute inset-0 flex items-center justify-between p-4 opacity-0 group-hover/gallery:opacity-100 transition-opacity"
            >
                <button
                    @click="prev"
                    class="w-10 h-10 rounded-full bg-white/90 hover:bg-white text-neutral-800 flex items-center justify-center shadow-lg border border-neutral-200 transition active:scale-95 cursor-pointer"
            ...
            <!-- Counter Tag -->
            <div
                class="absolute bottom-4 right-4 px-3 py-1 bg-neutral-900/75 backdrop-blur-xs text-white text-xs font-semibold rounded-full select-none"
            >
        ...
        <div v-if="resolvedImages.length > 1" class="grid grid-cols-5 gap-3">
            <button
                ...
                :class="
                    activeIndex === idx
                        ? 'border-brand-500 ring-2 ring-brand-100 scale-95'
