...
export const useSecurity = () => {
    ...
    const handleSendResetEmail = async () => {
        ...
        try {
            ...
            await $fetch(getApiUrl('/api/auth/password/email'), {
                method: 'POST',
    ...
    const handleResetPassword = async () => {
        ...
        try {
            securityStatusMsg.value = '';
            await $fetch(getApiUrl('/api/auth/password/reset'), {
                method: 'POST',
                body: {
    ...
    const handleSendVerifyEmail = async () => {
        ...
        try {
            securityStatusMsg.value = '';
            await $fetch(getApiUrl('/api/auth/email/send-verification'), {
                method: 'POST',
                body: { email: securityEmail.value },
            });
            securityStatusType.value = 'success';
    ...
    const handleVerifyEmailDirect = async () => {
        ...
        try {
            securityStatusMsg.value = '';
            await $fetch(getApiUrl('/api/auth/email/verify'), {
                method: 'POST',
                body: { email: securityEmail.value },
            });
            securityStatusType.value = 'success';
