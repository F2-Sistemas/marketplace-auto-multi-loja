...
<template>
    ...
    >
        ...
        <div
            v-if="icon && !title"
            class="absolute -right-4 -bottom-4 text-slate-800/20 text-8xl font-semibold select-none pointer-events-none"
        >
        ...
        <div v-if="title || $slots.header" class="mb-4 flex items-center justify-between">
            <slot name="header">
                <h3 class="text-base font-semibold text-white flex items-center gap-2">
                    <iconify-icon v-if="icon" :icon="icon" :class="iconColor || 'text-indigo-400'"></iconify-icon>
                    <span>{{ title }}</span>
                </h3>
            </slot>
        </div>
