...
const sizeClasses = computed(() => {
    switch (props.size) {
        ...
        case 'md':
            return 'px-5 py-2.5 text-sm font-normal';
        case 'lg':
            return 'px-6 py-3 text-base font-normal';
    }
});
...
<template>
    ...
    >
        <iconify-icon v-if="loading" icon="tabler:spinner" class="animate-spin text-sm"></iconify-icon>
        <iconify-icon v-else-if="icon" :icon="icon" class="text-sm"></iconify-icon>
        <slot></slot>
    </component>
</template>
