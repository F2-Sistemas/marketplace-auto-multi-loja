...
<template>
  <div class="min-h-screen bg-slate-950 text-slate-100 font-['Outfit'] antialiased flex flex-col md:flex-row">
    ...
    <!-- SIDEBAR NAVIGATION -->
    <aside class="w-full md:w-64 bg-slate-900 border-b md:border-b-0 md:border-r border-slate-800 flex flex-col justify-between">
      <div>
        <!-- BRAND LOGO -->
        <div class="p-6 flex items-center gap-3 border-b border-slate-800">
          <div class="w-9 h-9 rounded-xl bg-gradient-to-tr from-indigo-500 to-amber-400 flex items-center justify-center text-white shadow shadow-indigo-500/20">
            <iconify-icon icon="tabler:steering-wheel" class="text-xl"></iconify-icon>
          </div>
          <div>
            <span class="font-extrabold text-xl tracking-tight bg-gradient-to-r from-indigo-400 to-amber-400 bg-clip-text text-transparent">AutoHub</span>
            <span class="text-[9px] uppercase font-bold text-slate-400 block tracking-widest leading-none">Backoffice</span>
          </div>
        </div>
        ...
        <nav class="p-4 space-y-1.5">
          <button 
            ...
            :class="[
              ...
              activeTab === 'dashboard' ? 'bg-indigo-600 text-white shadow shadow-indigo-600/20' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-850'
            ]"
          >
          ...
          <button 
            ...
            :class="[
              'w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition',
              activeTab === 'stores' ? 'bg-indigo-600 text-white shadow shadow-indigo-600/20' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-850'
            ]"
          >
          ...
          <button 
            ...
            :class="[
              'w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition',
              activeTab === 'wizard' ? 'bg-indigo-600 text-white shadow shadow-indigo-600/20' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-850'
            ]"
          >
      ...
      <div class="p-4 border-t border-slate-800 bg-slate-950/40">
        <div class="flex items-center gap-3">
          <div class="w-9 h-9 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-indigo-400 font-bold text-sm">
            TS
          </div>
          <div>
    ...
    <main class="flex-1 p-6 md:p-8 space-y-8 max-w-7xl mx-auto w-full">
      ...
      <header class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <span class="px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-[10px] font-bold uppercase tracking-wider flex items-center gap-1.5">
      ...
      <div v-if="activeTab === 'dashboard'" class="space-y-8 animate-in fade-in duration-300">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 relative overflow-hidden">
            ...
            <span class="text-[10px] text-slate-500 uppercase tracking-wider font-extrabold block">Lojas Ativas</span>
          ...
          <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 relative overflow-hidden">
            ...
            <span class="text-[10px] text-slate-500 uppercase tracking-wider font-extrabold block">Veículos Anunciados</span>
          ...
          <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 relative overflow-hidden">
            ...
            <span class="text-[10px] text-slate-500 uppercase tracking-wider font-extrabold block">Propostas / Leads</span>
          ...
          <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 relative overflow-hidden">
            ...
            <span class="text-[10px] text-slate-500 uppercase tracking-wider font-extrabold block">Faturamento Mensal</span>
          ...
          <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 lg:col-span-2">
            ...
            <div class="space-y-4 mt-6">
              ...
              <div class="w-full h-2 bg-slate-800 rounded-full overflow-hidden">
                <div class="h-full bg-gradient-to-r from-indigo-500 to-indigo-400 rounded-full" style="width: 45%"></div>
              </div>
              ...
              <div class="w-full h-2 bg-slate-800 rounded-full overflow-hidden">
                <div class="h-full bg-gradient-to-r from-amber-400 to-amber-500 rounded-full" style="width: 35%"></div>
              </div>
              ...
              <div class="w-full h-2 bg-slate-800 rounded-full overflow-hidden">
                <div class="h-full bg-gradient-to-r from-emerald-450 to-emerald-400 rounded-full" style="width: 20%"></div>
              </div>
            </div>
          </div>
      ...
      <div v-if="activeTab === 'stores'" class="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden animate-in fade-in duration-300">
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-left border-collapse">
            <thead>
              <tr class="bg-slate-950/60 text-[10px] font-extrabold uppercase text-slate-500 tracking-wider border-b border-slate-800">
            ...
            <tbody class="divide-y divide-slate-850">
              ...
              >
                ...
                <td class="py-4 px-6">
                  <div class="flex items-center gap-3">
                    <div :class="['w-8 h-8 rounded-lg bg-slate-950 flex items-center justify-center font-bold text-xs', `text-${s.accentColor}-400`]">
                      <iconify-icon icon="tabler:building-store"></iconify-icon>
                    </div>
                    <div>
                ...
                <td class="py-4 px-6 text-xs text-indigo-400 font-semibold">{{ formatPlan(s.plan) }}</td>
                <td class="py-4 px-6 text-xs text-slate-400">{{ s.created_at }}</td>
                <td class="py-4 px-6 text-center">
                  <span 
                    :class="[
                      'px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider',
                      s.status === 'ativo' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'
                    ]"
                  >
                    {{ s.status }}
                  </span>
                ...
                <td class="py-4 px-6 text-right">
                  <button 
                    ...
                    :class="[
                      ...
                      s.status === 'ativo' ? 'bg-slate-800 hover:bg-rose-950/20 text-rose-400' : 'bg-indigo-600/10 hover:bg-indigo-600/20 text-indigo-400'
                    ]"
                  >
      ...
      <div v-if="activeTab === 'wizard'" class="max-w-xl mx-auto bg-slate-900 border border-slate-800 rounded-3xl p-6 md:p-8 animate-in fade-in duration-300">
        ...
        <div v-if="showWizardSuccess" class="bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 p-4 rounded-xl flex items-center gap-2 mb-6 animate-in fade-in duration-200">
        ...
        <form @submit.prevent="handleCreateStore" class="space-y-6">
          <div>
            <label class="block text-xs font-semibold text-slate-400 uppercase mb-1.5">Nome da Revenda</label>
            <input 
          ...
          <div>
            <label class="block text-xs font-semibold text-slate-400 uppercase mb-1.5">Subdomínio (Host)</label>
            <div class="flex">
              <input 
              ...
              />
              <span class="bg-slate-800 border border-l-0 border-slate-850 px-4 flex items-center text-xs text-slate-400 rounded-r-xl">
                .rederevenda.com
              </span>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
              <label class="block text-xs font-semibold text-slate-400 uppercase mb-1.5">Plano Assinatura</label>
              <select 
            ...
            <div>
              <label class="block text-xs font-semibold text-slate-400 uppercase mb-1.5">Cor Temática</label>
              <select 
